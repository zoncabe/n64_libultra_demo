...
void rotate_point_xyz(float point[3], float rotation[3]);
...
void rotate_point_zyx(float point[3], float rotation[3]) 
{
    // Rotate around Z axis
    float radZ = rad(rotation[2]);
    float xZ = point[0] * cosf(radZ) - point[1] * sinf(radZ);
    float yZ = point[0] * sinf(radZ) + point[1] * cosf(radZ);

    // Rotate around Y axis
    float radY = rad(rotation[1]);
    float xY = xZ * cosf(radY) + point[2] * sinf(radY);
    float zY = point[2] * cosf(radY) - xZ * sinf(radY);

    // Rotate around X axis
    float radX = rad(rotation[0]);
    point[1] = yZ * cosf(radX) - zY * sinf(radX);
    point[2] = yZ * sinf(radX) + zY * cosf(radX);
    point[0] = xY;
}
...
void point_global_to_obb_space(OBB* obb, float global_point[3], float local_point[3]) 
{
    ...
    rotate_point_zyx(local_point, inverse_rotation);
}
...
void point_obb_to_global_space(OBB* obb, float local_point[3], float global_point[3]) 
{
    ...
    float inverse_rotation[3] = {-obb->rotation[0], -obb->rotation[1], -obb->rotation[2]};
    rotate_point_xyz(local_point, inverse_rotation);
...
void get_obb_corners(OBB obb, float corners[8][3])
{
    ...
    for (int i = 0; i < 8; i++) {
        rotate_point_xyz(localCorners[i], obb.rotation);
...
int collision_obb_aabb(OBB obb, AABB aabb) {
    ...
    for (int i = 0; i < 3; i++) {
        float axis[3] = {0, 0, 0};
        axis[i] = 1;
        rotate_point_xyz(axis, obb.rotation);
        obbAxes[i][0] = axis[0];
        obbAxes[i][1] = axis[1];
        obbAxes[i][2] = axis[2];
    }
