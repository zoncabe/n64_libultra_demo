...
void set_collision_plane_aabb(Entity* entity, AABB* aabb)
{
    ...
    // Inicializar el punto del plano con la esquina más cercana al punto de colisión
    if (entity->collision.normal[0] != 0) {

        aabb->plane.point[0] = (entity->collision.normal[0] == -1) ? aabb->min[0] : aabb->max[0];
        aabb->plane.point[1] = (distance[2] < distance[3]) ? aabb->min[1] : aabb->max[1];
        aabb->plane.point[2] = 0;

        if (fabs(aabb->min[1] - entity->position[1]) < edge_threshold
            || entity->position[1] < aabb->min[1] 
            || fabs(aabb->max[1] - entity->position[1]) < edge_threshold 
            || entity->position[1] > aabb->max[1]) {

            set_vector(entity->collision.normal, aabb->plane.point, entity->position); 
        ...
        aabb->plane.point[0] = (distance[0] < distance[1]) ? aabb->min[0] : aabb->max[0];
        aabb->plane.point[1] = (entity->collision.normal[1] == -1) ? aabb->min[1] : aabb->max[1];
        aabb->plane.point[2] = 0;

        if (fabs(aabb->min[0] - entity->position[0]) < edge_threshold 
            || entity->position[0] < aabb->min[0] 
            || fabs(aabb->max[0] - entity->position[0]) < edge_threshold 
            || entity->position[0] > aabb->max[0]) {
        ...
        aabb->plane.point[0] = (distance[0] < distance[1]) ? aabb->min[0] : aabb->max[0];
        aabb->plane.point[1] = (distance[2] < distance[3]) ? aabb->min[1] : aabb->max[1];
        aabb->plane.point[2] = (entity->collision.normal[2] == -1) ? aabb->min[2] : aabb->max[2];
    }
...
void set_collision_point(Entity* entity, Capsule capsule, AABB aabb)
{
    ...
    for (int i = 0; i < 3; i++) {
        entity->previous_position[i] = entity->collision.point[i] + aabb.plane.normal[i] * capsule.radius;
    }
}
